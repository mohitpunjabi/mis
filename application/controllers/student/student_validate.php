...
class Student_validate extends MY_Controller
{
	...
	public function validation($admn_no)
	{
		if($admn_no)
		{
	...
	function details_rejected($stu_id)
	{
		...
		if(!$this->student_status_details_model->update($stu_status,$stu_id))
	...
	function delete_pending_data($stu_id)
	{
		...
		$stu_other_details=$this->student_other_details_model->get_pending_student_other_details_by_id($admn_no);
		$stu_fee_details=$this->student_fee_details_model->get_pending_stu_fee_details_by_id($admn_no);
		$user_other_details=$this->user_other_details_model->getPendingDetailsById($admn_no);
		$stu_details=$this->student_details_model->get_pending_student_details_by_id($admn_no);
		$user_permanent_address=$this->user_address_model->getPendingDetailsById($admn_no,'permanent');
		$user_present_address=$this->user_address_model->getPendingDetailsById($admn_no,'present');
		$user_correspondence_address=$this->user_address_model->getPendingDetailsById($admn_no,'correspondence');
		$stu_education_details = $this->student_education_details_model->getPendingStuEduById($admn_no);
		$stu_academic=$this->student_academic_model->get_pending_stu_academic_details_by_id($admn_no);
		$correspondence_address=$this->user_address_model->getDetailsById($admn_no,'correspondence');

		if($this->user_details_model->getUserById())
		{
			/*if($user_details)
		...
		else
		{
			...
			$this->user_details_model->inser($user_details);
		...
		$this->student_details_to_approve->delete($admn_no);
		...
		$this->student_details_model->deletePendingDetailsWhere(array('id'=>$stu_id));
