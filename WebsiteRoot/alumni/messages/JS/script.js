...
$(document).ready(function(){
	...
	var getCourses = function () {
		...
		$.ajax({
			url: "http://localhost/mis/WebsiteRoot/alumni/messages/module.php?a=getCourses",
			dataType: "json",
			cache: true,
			type: "post",
			success: function (response){
	            ...
	            var str = '<div>'+"\n";
	            // window.str = response;
	            utils.alertC(utils.enumerate(response), "Courses");
	            for(var x in response){
	            	str += "\t"+'<span style="display: block; margin: 3px;"><input type="radio" name="course[]" value="' + x + '" data-sem="'+response[x][1]+'">'+response[x][0]+'</input></span>'+"\n";
	            }
	            str += '</div>';
	            $(".course").show();
	            $("#course").html(str);
	...
	getBranches = function(){
		if($("input[name='groupby']")[0].checked)
			return;

		// var courses = JSON.stringify(window.selectedCourses);
		$.ajax({
			url: "http://localhost/mis/WebsiteRoot/alumni/messages/module.php?a=getBranch",
			dataType: "json",
			cache: true,
			type: "post",
			data: {course: window.selectedCourses},
			success: function (response){
	            ...
	            // utils.alertD(typeof response.length, typeof response.length);
	            if(response.length){
					...
					$("input[name='sem[]']").click(function(){
			        	window.selectedSemester.push(this.value);
			        });
		        } else {
		        	$(".branch").show();
					$("#branch").html("No Branches found");
					$(".sem").hide();
		        }
			}
		});
	...
	function sendMessage(){
		...
		str = "";
		$.ajax({
		    url: "http://localhost/mis/WebsiteRoot/alumni/messages/module.php?a=insertMsg",
			cache: true,
	...
	function showSearchForm(){
		$("#bulk").hide();
		$("tr.course-branch").hide();
		if(!searchFormExist){
			searchFormExist = true;
			searchForm = $('<div class="searchForm"><table><tr><td><input name="keyword" id="search-keyword" placeholder="Enter a name" style="width: 400px; min-height: 20px; border: 1px solid #ccc; padding: 5px;" autocomplete="off"/></td><td><input type="submit" name="search-user" value="Search users" onclick="searchName($(\'#search-keyword\').val(), true)"/></td></tr></table></div>');
		...
		searchName = function (keyword, button) {
			if(check(keyword)){
				...
				$.ajax({
					url: "http://localhost/mis/WebsiteRoot/alumni/messages/module.php?a=searchPeople&keyword="+keyword,
					dataType: "json",
					//cache: true,
					type: "post",
					data:{
					...
					success: function (response){
						...
						var responsetable = $('<ul class="responsetable" style="position: absolute; list-style: none; padding-top: 0px; border: 1px solid #ccc; padding-left: 0px; min-width: 300px; border-width: 1px 0px 0px 1px;"></ul>');
						...
						if(!length){
							responsetable.append($("<li style='padding: 3px 20px; border-bottom: 0px solid #ccc; background-color: #c33;' class='response-name'>No such name exists.</li>"));
						}
	...
	$("input[name='groupby']").change(function(){
		// utils.alertC($("input[name='groupby']")[1].checked);
		if($("input[name='groupby']")[0].checked){
			$(".branch").html("").hide();
			$(".sem").html("").hide();
		} else if($("input[name='groupby']")[1].checked){
			getBranches();
		}

	});
	addSpinner = function(el, pos, size){
		pos = (pos)? pos: "right";
		size = (size)? size: "32px";
		spinner = $('<div id="spinner" style="position: absolute; '+pos+': 100%;"></div>');

		spinner.append('<img src="http://localhost/mis/images/loading.gif" alt="loading" style=" width: '+size+'px; height: '+size+'px;" />')
		$(el).append(spinner);
	}
});